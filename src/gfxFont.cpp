...
bool gfxFont::init(u8 * buffer, int len)
{
	...
	SDL_Surface *fontsurf = Load_Textura_From_Mem(buffer, len);
	if (fontsurf == NULL)
	{
		printf(" ERROR: Couldn't load file %s\n ", SDL_GetError());
...
bool gfxFont::init()
{
	...
	// font mmx
	font_mmx = SFont_InitFont(Load_Textura_From_Mem((u8 *) font_mmx_data, font_mmx_size));

	// font large
	font_large = SFont_InitFont(Load_Textura_From_Mem((u8 *) font_large_data, font_large_size));

	// font rock
	m_font = SFont_InitFont(Load_Textura_From_Mem((u8 *) rock_font_data, rock_font_size));

	// font small
	font_small = SFont_InitFont(Load_Textura_From_Mem((u8 *) font_small_data, font_small_size));
